...
import sys
sys.path.append("/home/ex9845/ws/SSD/models")
...
class ResNet(nn.Module):
    def __init__(self, backbone='resnet50', backbone_path=None,prune=False):
        super().__init__()
        if prune:
            print("prune!")
            dict_ = torch.load(backbone_path,map_location='cuda:0')
            cfg = dict_["cfg"]
            if backbone == 'resnet18':
                backbone = models.resnet18(cfg=cfg)
                backbone.load_state_dict(dict_["state_dict"])
                self.out_channels = [256, 512, 512, 256, 256, 128]
            elif backbone == 'resnet34':
                backbone = models.resnet34(cfg=cfg)
                backbone.load_state_dict(dict_["state_dict"])
                self.out_channels = [256, 512, 512, 256, 256, 256]
            elif backbone == 'resnet50':
                backbone = models.resnet50(cfg=cfg)
                backbone.load_state_dict(dict_["state_dict"])
        ...
        conv4_block1.convShortcut.stride = (1, 1)
